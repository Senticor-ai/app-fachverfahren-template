...
// useIsMobile — inline, damit der Kit ohne externen Hooks-Pfad self-contained bleibt.
...
>(
  (
    ...
    // Interner State der Sidebar. openProp/setOpenProp erlauben Steuerung von außen.
    ...
    const setOpen = React.useCallback(
      (value: boolean | ((value: boolean) => boolean)) => {
        ...
        // Setzt das Cookie, um den Sidebar-Zustand zu behalten.
    ...
    // Tastatur-Shortcut zum Umschalten.
    React.useEffect(() => {
    ...
    // data-state="expanded"|"collapsed" zur Tailwind-Stilsteuerung.
  ...
  (
    ...
    return (
      <div
        ...
        data-side={side}
      >
        {/* Behandelt den Sidebar-Spalt auf Desktop */}
        <div
          className={cn(
            "relative w-(--sidebar-width) bg-transparent transition-[width] duration-(--fv-duration-slow) ease-linear",
        ...
        <div
          className={cn(
            "fixed inset-y-0 z-10 hidden h-svh w-(--sidebar-width) transition-[left,right,width] duration-(--fv-duration-slow) ease-linear md:flex",
            side === "left"
              ? "left-0 group-data-[collapsible=offcanvas]:left-[calc(var(--sidebar-width)*-1)]"
              : "right-0 group-data-[collapsible=offcanvas]:right-[calc(var(--sidebar-width)*-1)]",
            // Padding für floating/inset-Varianten anpassen.
            variant === "floating" || variant === "inset"
...
>(({ className, asChild = false, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      className={cn(
        "flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium text-sidebar-foreground/70 outline-none ring-sidebar-ring transition-[margin,opacity] duration-(--fv-duration-slow) ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
  ...
  return (
    <Comp
      ...
      className={cn(
        ...
        // Vergrößert die Trefferfläche auf Mobil.
...
>(({ className, showIcon = false, ...props }, ref) => {
  // Zufällige Breite zwischen 50 und 90%.
