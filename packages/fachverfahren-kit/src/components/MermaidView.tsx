...
function sweepMermaidOrphans(): void {
  ...
  try {
    document.querySelectorAll('svg[aria-roledescription="error"], svg[id^="mermaid-"], div[id^="dmermaid"], .error-text').forEach((n) => {
...
function loadMermaid() {
  if (!mermaidPromise) {
    mermaidPromise = import("mermaid").then(async (m) => {
      ...
      try { const elk = await import("@mermaid-js/layout-elk"); m.default.registerLayoutLoaders(elk.default); } catch { /* dagre-Fallback */ }
      m.default.initialize({
        ...
        themeVariables: {
          ...
          secondaryColor: "#e2e8f0", tertiaryColor: "#f8fafc", lineColor: "#64748b", textColor: "#1e293b",
          mainBkg: "#f8fafc", clusterBkg: "#f8fafc", clusterBorder: "#cbd5e1", edgeLabelBackground: "#ffffff",
          titleColor: "#0f172a", actorBkg: "#eef2f7", actorBorder: "#94a3b8", noteBkgColor: "#fef9c3", noteBorderColor: "#facc15",
        },
        flowchart: { htmlLabels: false, useMaxWidth: true, nodeSpacing: 54, rankSpacing: 64, padding: 18, diagramPadding: 12, curve: "basis" },
        sequence: { useMaxWidth: true, boxMargin: 10, noteMargin: 10, messageMargin: 38, mirrorActors: false },
      ...
      (async () => {
        try {
          ...
          try {
            ...
            sweepMermaidOrphans();
            ({ svg: raw } = await mermaid.render(`mmd-${id}-dagre`, `%%{init: {"layout":"dagre"}}%%\n${normalized}`));
          }
...
function MermaidZoomModal({ svg, onClose }: { svg: string; onClose: () => void }) {
  ...
  const onWheel = (e: ReactWheelEvent) => { e.preventDefault(); setScale((s) => Math.min(8, Math.max(0.3, s * (e.deltaY < 0 ? 1.12 : 0.89)))); };
  const onDown = (e: ReactMouseEvent) => { drag.current = { x: e.clientX, y: e.clientY, tx, ty }; };
  const onMove = (e: ReactMouseEvent) => { const d = drag.current; if (!d) return; setTx(d.tx + (e.clientX - d.x)); setTy(d.ty + (e.clientY - d.y)); };
  const onUp = () => { drag.current = null; };

  return (
    <div className="fixed inset-0 z-[100] flex flex-col bg-black/70 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Diagramm Vollbild" onClick={onClose}>
      <div className="flex items-center gap-2 border-b border-white/10 bg-surface px-4 py-2" onClick={(e) => e.stopPropagation()}>
        <span className="text-[12px] font-medium text-foreground">Diagramm · Vollbild</span>
        <div className="ml-auto flex items-center gap-1">
          <button onClick={() => setScale((s) => Math.max(0.3, s * 0.8))} className="rounded px-2 py-1 text-[13px] text-muted-foreground hover:bg-surface-2 hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" title="Verkleinern" aria-label="Verkleinern">−</button>
          <span className="w-12 text-center text-[11px] tabular-nums text-muted-foreground">{Math.round(scale * 100)}%</span>
          <button onClick={() => setScale((s) => Math.min(8, s * 1.25))} className="rounded px-2 py-1 text-[13px] text-muted-foreground hover:bg-surface-2 hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" title="Vergrößern" aria-label="Vergrößern">+</button>
          <button onClick={reset} className="rounded px-2 py-1 text-[11px] text-muted-foreground hover:bg-surface-2 hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" title="Zurücksetzen">Reset</button>
          <button onClick={onClose} className="ml-1 rounded px-2 py-1 text-[11px] text-muted-foreground hover:bg-surface-2 hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" title="Schließen (Esc)" aria-label="Schließen">✕</button>
        </div>
      </div>
      <div
      ...
      >
        <div
          ...
          style={{ transform: `translate(${tx}px, ${ty}px) scale(${scale})`, transformOrigin: "center center", transition: drag.current ? "none" : "transform 0.08s" }}
          dangerouslySetInnerHTML={{ __html: svg }}
        />
      </div>
    </div>
