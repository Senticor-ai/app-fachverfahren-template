...
// Audit-Trail). Abgeleitet 1:1 aus der Referenz `amt.vorgang.$id.tsx` (lovable): KI-Vorschlag-Karte, strukturierte
// Datensektionen, Audit-Trail. ABER streng config-getrieben: die Sektionen kommen aus `config.detailSektionen`
// (verschachtelte Pfade in `vorgang.antragsdaten`), nicht aus Domänen-Literalen. Kein "Hund"/"Halter" hartkodiert —
// ein zweites Verfahren (Gewerbe/Parkausweis/Bauantrag) rendert unverändert.
...
function Sektion<T>({ sektion, antragsdaten }: { sektion: DetailSektion; antragsdaten: T }) {
    ...
    <section className="overflow-hidden rounded-md border border-status-info/30 bg-status-info-soft">
      <div className="flex items-start justify-between gap-4 p-5">
        <div className="min-w-0">
          <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wide text-status-info">
          ...
          {b && (
            <>
              ...
              <p className="mt-1 text-sm font-medium text-foreground">{b.label}</p>
              <p className="mt-2 text-sm text-foreground">{b.begruendung}</p>
              {b.positionen && b.positionen.length > 0 && (
                <ul className="mt-3 space-y-1 border-t border-status-info/20 pt-3 text-[12px]">
                  {b.positionen.map((p, i) => (
                    <li key={i} className="flex items-center justify-between gap-4 text-foreground">
                      <span>{p.label}</span>
      ...
      <ol className="mt-3 space-y-3">
        {history.map((h, i) => (
          <li key={i} className="relative pl-4 text-[12px]">
            ...
            {h.detail && <div className="text-[11px] text-foreground/80">{h.detail}</div>}
            <div className="text-[11px] text-muted-foreground">
              {new Date(h.ts).toLocaleString("de-DE", { dateStyle: "short", timeStyle: "short" })} ·{" "}
              {h.rolle}
            </div>
          </li>
        ))}
...
function StrukturierteUebergabe({ vorgang }: { vorgang: Vorgang }) {
  const schema = {
    ...
    ...(vorgang.berechnung
      ? { ergebnis: { betrag: vorgang.berechnung.betrag, einheit: vorgang.berechnung.einheit } }
      : {}),
  };
  return (
    <section className="rounded-md border border-border bg-card p-5">
      ...
      <p className="mt-1 text-[12px] text-muted-foreground">
        Diese Vorgangsdaten gehen — kein manuelles Abtippen — als Schema an das Fachverfahren.
      </p>
      <pre className="mt-3 overflow-x-auto rounded-sm border border-border bg-muted p-3 font-mono text-[11px] leading-relaxed text-foreground">
  ...
  return (
    <div className={cn("space-y-6", className)}>
      ...
      {config.detailSektionen.map((sektion, i) => (
        <Sektion key={i} sektion={sektion} antragsdaten={vorgang.antragsdaten} />
      ))}

      {zeigeUebergabe && <StrukturierteUebergabe vorgang={vorgang as Vorgang} />}
