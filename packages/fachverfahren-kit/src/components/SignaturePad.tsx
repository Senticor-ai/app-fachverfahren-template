...
export function SignaturePad({
  ...
  const meldeAenderung = useCallback(() => {
    ...
    setHatInhalt(!leer);
    setStatusMeldung(leer ? "Unterschriftenfeld ist leer." : "Unterschrift erfasst.");
    if (!onChange) return;
    if (leer) {
      onChange(null);
      return;
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <h2 className="flex items-center gap-2 text-base font-semibold text-foreground">
            <SignatureIcon className="h-4 w-4 text-muted-foreground" aria-hidden="true" />
            {titel}
          </h2>
        ...
        <span
          className={cn(
            "inline-flex shrink-0 items-center gap-1 rounded-sm border px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-wide",
            hatInhalt
        ...
        >
          <PenLine
            className={cn("h-3 w-3", hatInhalt ? "text-status-ok" : "text-muted-foreground")}
            aria-hidden="true"
          />
      ...
      <p
        id={hinweisId}
        className="mt-3 flex items-start gap-2 rounded-sm border border-border bg-background p-3 text-[12px] text-muted-foreground"
      >
