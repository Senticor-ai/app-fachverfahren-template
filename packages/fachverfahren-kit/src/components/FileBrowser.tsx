...
import { ChevronRight, Folder, FolderOpen, File as FileIcon, Search, Download, Trash2, Pencil } from "lucide-react";
  ...
  const walk = (list: FileNode[]): FileNode[] =>
    list.flatMap((n) => {
      if (n.type === "folder") {
        ...
        if (kids.length || n.name.toLowerCase().includes(ql)) return [{ ...n, children: kids }];
        return [];
      }
...
export function FileBrowser({ nodes, onOpen, onDownload, onDelete, onRename, title = "Dateien", emptyHint = "Keine Dateien vorhanden.", className }: Props) {
  ...
  const Row = ({ node, depth }: { node: FileNode; depth: number }) => {
    ...
    return (
      <li role="treeitem" aria-expanded={isFolder ? isOpen : undefined} aria-selected={isSel}>
        <div
          className={cn(
            "group flex items-center gap-1.5 rounded-md py-1 pr-1.5 text-[13px] transition-colors duration-150 ease-out motion-reduce:transition-none",
            isSel ? "bg-accent text-foreground" : "hover:bg-accent/60 text-foreground/90",
          )}
          style={{ paddingLeft: 6 + depth * 16 }}
        >
          <button
            type="button"
            onClick={() => { setSelected(node.id); if (isFolder) toggle(node.id); else onOpen?.(node); }}
          ...
          >
            ...
            {!isFolder && (node.size != null || node.modified) && (
              <span className="ml-2 shrink-0 text-[11px] text-muted-foreground tabular-nums">
                {[formatSize(node.size), node.modified].filter(Boolean).join(" · ")}
              </span>
            )}
          </button>
          {!isFolder && (
            <span className="flex shrink-0 items-center gap-0.5 opacity-0 transition-opacity group-hover:opacity-100 group-focus-within:opacity-100">
              {onDownload && <IconBtn icon={Download} label="Herunterladen" onClick={() => onDownload(node)} />}
              {onRename && <IconBtn icon={Pencil} label="Umbenennen" onClick={() => onRename(node)} />}
              {onDelete && <IconBtn icon={Trash2} label="Löschen" onClick={() => onDelete(node)} danger />}
            </span>
          )}
        </div>
      ...
      <div className="flex items-center gap-2 border-b border-border px-3 py-2">
        ...
        <div className="relative ml-auto">
          ...
          <input
            ...
            className="h-8 w-40 rounded-md border border-input bg-background pl-7 pr-2 text-[12px] text-foreground outline-none focus-visible:ring-2 focus-visible:ring-ring placeholder:text-muted-foreground"
          />
        </div>
      </div>
      {filtered.length === 0 ? (
        <div className="px-3 py-8 text-center text-[13px] text-muted-foreground">{query ? "Keine Treffer." : emptyHint}</div>
      ) : (
        <ul role="tree" aria-label={title} className="max-h-[28rem] list-none overflow-auto p-1.5">
          {filtered.map((n) => <Row key={n.id} node={n} depth={0} />)}
        </ul>
      )}
    </div>
  );
...
function IconBtn({ icon: Icon, label, onClick, danger }: { icon: typeof Download; label: string; onClick: () => void; danger?: boolean }) {
  return (
    <button
      type="button"
