// fachverfahren-kit/components/KiVorschlag — KI-Transparenz-Block, abgeleitet aus der Referenz `AIBadge.tsx`
// (AIBadge + ConfidenceBar, lovable). GENERISCH: konsumiert NUR die `KiEinschaetzung` aus dem Vertrag
...
function ConfidenceBar({ value }: { value: number }) {
  ...
  const tone = value >= 0.85 ? "bg-status-ok" : value >= 0.7 ? "bg-accent" : "bg-status-warn";
  return (
...
function FlagBadge({ children }: { children: string }) {
  return (
    <span className="inline-flex items-center gap-1 rounded-sm bg-status-info-soft px-1.5 py-0.5 text-[11px] font-medium text-status-info">
...
export function KiVorschlag({ ki, schwelleAutonom, flagLabel, className }: KiVorschlagProps) {
  // „Autonom-fähig": hohe Konfidenz UND keine offenen Flags (nur wenn eine Schwelle vorgegeben ist).
  const autonom = schwelleAutonom !== undefined && ki.confidence >= schwelleAutonom && ki.flags.length === 0;
    ...
    >
      <div className="flex items-center justify-between gap-2">
        <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
          <Sparkles className="h-3.5 w-3.5 text-status-info" aria-hidden="true" />
          KI-Vorschlag
        </span>
        {autonom && (
          <span className="inline-flex items-center gap-1 rounded-sm bg-status-ok-soft px-1.5 py-0.5 text-[10px] font-bold text-status-ok">
      ...
      {ki.flags.length > 0 && (
        <div className="mt-3 space-y-1.5">
          <span className="block text-[10px] font-bold uppercase tracking-wide text-muted-foreground/70">
            Hinweise der KI
          </span>
          <div className="flex flex-wrap gap-1.5">
            {ki.flags.map((flag) => (
              <FlagBadge key={flag}>{flagLabel ? flagLabel(flag) : flag}</FlagBadge>
            ))}
          </div>
        </div>
      )}
      ...
      {ki.begruendung && (
        <div className="mt-3 space-y-1">
          ...
          <p className="text-xs leading-relaxed text-foreground">{ki.begruendung}</p>
        </div>
      )}
    </div>
  );
