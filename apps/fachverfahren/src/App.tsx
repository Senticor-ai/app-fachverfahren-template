// App = die KOMPOSITION. Hier ist NULL fachliche Logik und KEIN Hundesteuer-Screen — nur:
//   1. Routing (react-router): URL → Persona → Kit-Baustein.
//   2. Die EINE Store-Instanz aus ./store (Kit-Store + Hundesteuer-Config) als `port` an jeden Baustein.
...
// kommt aus den Kit-Bausteinen + der Config. Tausche die Config in ./store → dieselbe App, anderes Verfahren.
      ...
      <div className="mx-auto max-w-2xl p-4 md:p-8">
        {v ? (
          <div className="rounded-lg border border-status-ok/30 bg-status-ok-soft p-6">
            <h1 className="text-lg font-semibold text-foreground">Antrag eingegangen</h1>
            <p className="mt-2 text-sm text-muted-foreground">
              Ihr Vorgang wurde unter der Nummer{" "}
              <span className="font-mono font-medium text-foreground">{v.vorgangsnummer}</span>{" "}
...
function AmtEingang(): React.JSX.Element {
  ...
  return (
    <Shell persona="sachbearbeitung" activeNavKey="eingang">
      <Arbeitsvorrat config={store.config} port={store} onOpen={(id) => navigate(`/amt/vorgang/${id}`)} />
    </Shell>
  );
}
