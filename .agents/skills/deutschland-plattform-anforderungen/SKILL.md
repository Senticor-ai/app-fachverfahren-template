...
Diese Skill ist das Runbook für das deutsche Anforderungenpaket rund um Deutschlandplattform, SaaS-Readiness, CHOS-BMS-Sicherheitskonzept und BSI-IT-Grundschutz-Kontext. Das Ziel ist ein reproduzierbares deutsches Paket mit klarer Provenienz, sauberer Plattformdelegation und lokal downloadbaren Exporten.

## Quellen und Artefakte

Kanonische Quelle ist `docs/platform-requirements/requirements.json` (Manifest) plus die Anforderungs-Shards unter `docs/platform-requirements/requirements/`. Das Manifest führt alle Abschnitte außer der Anforderungsliste; an deren Stelle steht `requirement_shards`, ein geordneter Index der Shard-Dateien. Die Anforderungen sind je Quelle geshardet, Grundschutz zusätzlich je Baustein — so reißt keine Einzeldatei das 1-MiB-Repo-Hygiene-Limit (`scripts/pr-hygiene-check.sh`). Der Renderer ist `scripts/render-platform-requirements.py`, bleibt stdlib-only, setzt die Shards beim Lesen wieder in Listenreihenfolge zusammen und schreibt sie auf `--refresh-json` neu. Eine Anforderung manuell ändern heißt: ihren Shard unter `requirements/` editieren (nicht das Manifest) und danach exportieren.
...
- Harte Blocker nur für souveränes KMS/Schlüsselmanagement, plattformbereitgestellte Inferenz und echte CHOS-BMS-Audit-`blockingFindings` setzen.
...
- `Anbieter` nur verwenden, wenn die Verantwortung wirklich Senticor-geführt bleibt. Bei KI-Inventar, Modellnachweisen, SBOM, Signing, Scanning, Deployment, Change und Auditfeeds ist meist `Gemeinsam` richtig.
