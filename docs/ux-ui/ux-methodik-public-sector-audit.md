...
| HCAI als Entscheidungslinse                     |               Teilweise | Screen Contract und Skill verlangen KI-Kennzeichnung, Quelle, Konfidenz, Override und Audit. KI-Assistenz ist GEBAUT (`LeistungConfig.ki`, `POST /api/ai/assist`); die Risikoklasse je Funktion bleibt fachlich zu bewerten. |
| Personas und Touchpoints                        |    Erfüllt für Template | Login trennt Bürgerin und Sachbearbeitung; `/aufsicht` ist eine eigene Persona-Route (`apps/fachverfahren/src/app/route-gates.ts:42`), Management/Audit also ein eigener Strang.                                             |
...
| ------------------------------------ | ------------------: | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A Stack, Daten, Stabilität           |             Erfüllt | Node 24, strict ESM, React, Tailwind, shadcn-Primitives, deterministische Demo-Daten aus der `LeistungConfig`-Naht. Postgres ist der produktive Datenpfad (`PostgresAppStore`, `CaseStore`); MSW ist ausschließlich Test-Schicht (`pnpm run test:browser`). |
...
| H Info-/Hilfe-Seite                  |             Erfüllt | Das öffentliche Doku-Wiki `/hilfe` (`apps/fachverfahren/src/app/route-gates.ts:20`, `src/pages/hilfe.tsx`) rendert README, AGENTS, `docs/**` und die Skills — für Mensch UND KI-Agent, ohne Anmeldung.                                                      |
