...
| HCAI/KI-Patterns                                      |              Erfüllt | `LeistungConfig.ki` (`assist`/`chat`/`voice`) treibt `KiAssistPanel`/`AssistentPanel` mit den 5 Transparenzelementen; serverseitig `POST /api/ai/assist` und `POST /api/composables/:id/chat`. Kennzeichnung, Quelle, Konfidenz, Warum, Override und Audit gehören zu jedem KI-Beitrag. |
...
| Manual-Callout                               |              Status | App-/Storybook-Nachweis                                                                                                                                                                                                                                                                                                                   |
| -------------------------------------------- | ------------------: | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Mobile-first, geführt, ein Fokus pro Schritt | Erfüllt im Template | Die Bürger-Routen rendern config-getrieben aus der Naht: `/buerger`, `/buerger/anmelden`, `/buerger/bestaetigung/:id`, `/buerger/antraege`, `/buerger/antrag/:id`, `/buerger/bescheid/:id`, `/buerger/postfach` (`apps/fachverfahren/src/app/routes.tsx`). Der Formular-Stepper ist als Storybook- und Screen-Contract-Pflicht vorhanden. |
