...
`/internal/*` darf nie ueber den public Ingress geroutet werden. Der Chart
...
- `/startupz`: Server gebootet, Konfiguration geparst, statisches Bundle
  lesbar.
- `/livez`: Event Loop und Prozess reagieren; keine Dependency-Pruefung.
- `/readyz`: false waehrend Shutdown; prueft nur deklarierte erforderliche
  Upstreams.
...
Nach Rollout gehoeren Smoke-Tests gegen `/readyz`, `/runtime-config.json` und
...
`pnpm run check:k8s-delivery` rendert den Helm-Chart, prueft Pflichtobjekte und
laeuft gegen `kubeconform` und `conftest`. `pnpm run test:supply-chain`
...
ersten Pflicht-Gates, solange Registry/OIDC-Bindings nicht ueberall verfuegbar
sind.
